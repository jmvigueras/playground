...
module "r1_hub_azure_core_config" {
  ...
  fgt_active_extra-config  = join("\n", data.template_file.r1_hub_azure_core_s2s_config.*.rendered, [data.template_file.r1_hub_azure_core_s2s_l3.rendered])
  fgt_passive_extra-config = join("\n", data.template_file.r1_hub_azure_core_s2s_config.*.rendered, [data.template_file.r1_hub_azure_core_s2s_l3.rendered])

  vpc-spoke_cidr = [local.r1_hub_azure_spoke_vnet_cidrs]
}
data "template_file" "r1_hub_azure_core_s2s_config" {
  ...
  vars = {
    ...
    local_gw          = local.r1_hub_azure_core_h2h[count.index]["vpn_port"] == "private" ? local.r1_hub_azure_core_ilb_ip : ""
    vpn_aggr_id       = "${local.r1_hub_azure_core_h2h[count.index]["id"]}-ipsec"
    vpn_intf_id       = "${local.r1_hub_azure_core_h2h[count.index]["id"]}-ipsec_${count.index + 1}"
...
data "template_file" "r1_hub_azure_core_s2s_l3" {
  template = file("./templates/fgt_s2s_l3.conf")
  vars = {
    aggr_intf      = "${local.r1_hub_azure_core_h2h[0]["id"]}-ipsec"
    aggr_intf_ip   = cidrhost(cidrsubnet(local.r1_to_r1_cidr, 2, 0), 10)
    aggr_intf_mask = cidrnetmask(cidrsubnet(local.r1_to_r1_cidr, 2, 0))
    bgp_peer_asn   = local.r1_hub_on_prem_bgp_asn
    bgp_peer_ip    = cidrhost(cidrsubnet(local.r1_to_r1_cidr, 2, 0), 11)
...
module "r1_hub_vnet_spoke" {
  ...
  vnet_spoke_cidr = local.r1_hub_azure_spoke_vnet_cidrs
...
module "r1_hub_vnet_spoke_vm" {
  ...
  prefix                   = "${local.prefix}-r1-hub-${count.index + 1}"
  location                 = local.region_1
  resource_group_name      = local.rg_name == null ? azurerm_resource_group.r1_rg[0].name : local.rg_name
  tags                     = local.tags
  storage-account_endpoint = local.storage-account_endpoint == null ? azurerm_storage_account.r1_storageaccount[0].primary_blob_endpoint : local.storage-account_endpoint
  ...
  ni_ip       = cidrhost(module.r1_hub_vnet_spoke.subnet_cidrs["subnet_1"], 10 + count.index)

  user_data = data.template_file.r1_hub_vnet_spoke_vm.rendered
}
