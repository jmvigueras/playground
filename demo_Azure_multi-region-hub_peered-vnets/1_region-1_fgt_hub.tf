...
module "r1_fgt_hub_config" {
  ...
  */
  ...
  vpc-spoke_cidr = concat(local.r1_hub_vnet_spoke_cidrs, [module.r1_fgt_hub_vnet.subnet_cidrs["bastion"]])
}
...
resource "azurerm_subnet_route_table_association" "r1_rta-spoke-subnet-1" {
  subnet_id      = module.r1_hub_vnet_spoke.subnet_ids["subnet_1"][0]
  route_table_id = azurerm_route_table.r1_hub_vnet_spoke_rt.id
}
// Route table association
resource "azurerm_subnet_route_table_association" "r1_rta-spoke-subnet-2" {
  subnet_id      = module.r1_hub_vnet_spoke.subnet_ids["subnet_2"][0]
  route_table_id = azurerm_route_table.r1_hub_vnet_spoke_rt.id
}

#--------------------------------------------------------------------------------
# Create peering vNet FGT HUBs r1 and r2
#--------------------------------------------------------------------------------
resource "azurerm_virtual_network_peering" "peer-hubs-1" {
  name                      = "${local.prefix}-peer-to-hub-r1"
  resource_group_name       = local.r1_resource_group_name == null ? azurerm_resource_group.r1_rg[0].name : local.r1_resource_group_name
  virtual_network_name      = module.r1_fgt_hub_vnet.vnet["name"]
  remote_virtual_network_id = module.r2_fgt_hub_vnet.vnet["id"]
  allow_forwarded_traffic   = true
}
resource "azurerm_virtual_network_peering" "peer-hubs-2" {
  name                      = "${local.prefix}-peer-to-hub-r2"
  resource_group_name       = local.r2_resource_group_name == null ? azurerm_resource_group.r2_rg[0].name : local.r2_resource_group_name
  virtual_network_name      = module.r2_fgt_hub_vnet.vnet["name"]
  remote_virtual_network_id = module.r1_fgt_hub_vnet.vnet["id"]
  allow_forwarded_traffic   = true
}
