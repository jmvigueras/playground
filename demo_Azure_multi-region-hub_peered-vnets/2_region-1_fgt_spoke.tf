...
#--------------------------------------------------------------------------------
# Create peering vNet FGT HUBs in region r1 and r2
#--------------------------------------------------------------------------------
resource "azurerm_virtual_network_peering" "r1-peer-spoke-1" {
  count                     = local.spoke_number
  name                      = "r1-peer-spoke-${count.index + 1}-to-hub"
  resource_group_name       = local.r1_resource_group_name == null ? azurerm_resource_group.r1_rg[0].name : local.r1_resource_group_name
  virtual_network_name      = module.r1_fgt_spoke_vnet[count.index].vnet["name"]
  remote_virtual_network_id = module.r1_fgt_hub_vnet.vnet["id"]
  allow_forwarded_traffic   = true
}
resource "azurerm_virtual_network_peering" "r1-peer-spoke-2" {
  count                     = local.spoke_number
  name                      = "r1-peer-spoke-${count.index + 1}-to-hub"
  resource_group_name       = local.r1_resource_group_name == null ? azurerm_resource_group.r1_rg[0].name : local.r1_resource_group_name
...
resource "azurerm_virtual_network_peering" "r2-peer-spoke-1" {
  count                     = local.spoke_number
  name                      = "r2-peer-spoke-${count.index + 1}-to-hub"
  resource_group_name       = local.r1_resource_group_name == null ? azurerm_resource_group.r1_rg[0].name : local.r1_resource_group_name
  virtual_network_name      = module.r1_fgt_spoke_vnet[count.index].vnet["name"]
  remote_virtual_network_id = module.r2_fgt_hub_vnet.vnet["id"]
  allow_forwarded_traffic   = true
}
resource "azurerm_virtual_network_peering" "r2-peer-spoke-2" {
  count                     = local.spoke_number
  name                      = "r2-peer-spoke-${count.index + 1}-to-hub"
