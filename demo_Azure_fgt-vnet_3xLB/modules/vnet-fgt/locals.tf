locals {
  ...
  subnet_erc_cidr = cidrsubnet(var.vnet-fgt_cidr, 3, 7)
  ...
  fgt_1_ni_erc_ip     = cidrhost(local.subnet_erc_cidr, 10)
  ...
  fgt_2_ni_erc_ip     = cidrhost(local.subnet_erc_cidr, 11)
  ...
  fgt_1_ni_erc_name     = "${var.prefix}-ni-active-erc"
  ...
  fgt_2_ni_erc_name     = "${var.prefix}-ni-passive-erc"
