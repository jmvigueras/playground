...
// Create storage account if not provided
resource "azurerm_storage_account" "r3_storageaccount" {
  count                    = local.storage-account_endpoint == null ? 1 : 0
  name                     = "r3stgra${random_id.randomId[count.index].hex}"
...
// Create Resource Group if it is null
resource "azurerm_resource_group" "r3_rg" {
  count    = local.r2_resource_group_name == null ? 1 : 0
  name     = "${local.prefix}-r3-rg"
  location = local.region_3
