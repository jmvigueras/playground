...
resource "azurerm_virtual_machine" "fgt-passive" {
  count                        = var.fgt-passive-ni_ids != null && var.hub["ha"] ? 1 : 0
  name                         = "${var.prefix}-${var.hub["id"]}-fgt-passive"
  ...
  storage_os_disk {
    name              = "${var.prefix}-${var.hub["id"]}-osDisk-fgt-passive"
    caching           = "ReadWrite"
  ...
  # Log data disks
  storage_data_disk {
    name              = "${var.prefix}-${var.hub["id"]}-datadisk-fgt-passive"
    managed_disk_type = "Standard_LRS"
    create_option     = "Empty"
    lun               = 0
    disk_size_gb      = "30"
...
data "template_file" "fgt-passive_all-config" {
  count    = var.fgt-subnet_cidrs != null && var.fgt-passive-ni_ids != null && var.hub["ha"] ? 1 : 0
